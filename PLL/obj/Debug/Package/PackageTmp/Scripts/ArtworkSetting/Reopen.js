var table_reassign;
var first_load = true;   // by aof reopen
$(document).ready(function () {
    ...
    bind_lov_no_ajax('.cls_form_reopen .cls_workflow_type', '', '');
    bind_lov('.cls_form_reopen .cls_current_assign', '/api/lov/user', 'data.DISPLAY_TXT');
    ...
    $(".cls_form_reopen .cls_btn_clear").click(function () {
        $('.cls_form_reopen input[type=text]').val('');
        $('.cls_form_reopen .cls_workflow_type').val('All').change();
    });
    ...
    table_reassign = $('#table_reassign').DataTable({
        ...
        ajax: function (data, callback, settings) {
            $.ajax({
                url: suburl + "/api/wfsetting/reopen/info?data.wf_no=" + $('.cls_form_reopen .cls_workflow_number').val()
                    + "&data.wf_type=" + $('.cls_form_reopen .cls_workflow_type').val()
                    + "&data.create_date_from=" + $('.cls_form_reopen .cls_create_date_from').val()
                    + "&data.create_date_to=" + $('.cls_form_reopen .cls_create_date_to').val()
                    + "&data.current_user_id=" + $('.cls_form_reopen .cls_current_assign').val()
                    + "&data.first_load=" + first_load  // by aof reopen
                ,
                type: 'GET',
                data: data,
    ...
    $("#popup_reassign form").submit(function (e) {
        if ($(this).valid()) {
            myAjaxConfirmSubmitBlank(callbackSubmitReOpen);
        }
        $(this).removeAttr("novalidate");
        e.preventDefault();	//STOP default action
    });

    $(".cls_form_reopen .cls_btn_reassign").click(function () {
        if ($('.cls_form_reopen .cls_chk_reassign_wf:checked').length > 0) {

            $('.cls_form_reopen .cls_input_remark').val('');
            $('.cls_form_reopen .cls_lov_assign_to').val(null).trigger('change');
    ...
    $('.cls_form_reopen .cls_current_assign').append('<option selected = "selected" value="' + UserID + '">' + UserName + '</option>');
    first_load = false;   // by aof reopen

});

function reload_table_reopen() {
    $('#popup_reassign').modal('hide');
    table_reassign.ajax.reload();
}


function callbackSubmitReOpen() {
    var jsonObj = new Object();
    var data = [];
    jsonObj.data = {};

    $(".cls_form_reopen .cls_chk_reassign_wf:checked").each(function () {
        var item = {};
        item["WORKFLOW_SUB_ID"] = $(this).data("wf_sub_id");
        item["WORKFLOW_TYPE"] = $(this).closest('tr').find('.cls_td_wf_type').text();
        item["CURRENT_USER_ID"] = $('.cls_form_reopen .cls_lov_assign_to').val();
        item["REMARK_REASSIGN"] = $('.cls_form_reopen .cls_input_remark').val();
    ...
    var myurl = '/api/wfsetting/reopen/info';
    var mytype = 'POST';
    var mydata = jsonObj;
    
    myAjaxAlertNoBack(myurl, mytype, mydata, reload_table_reopen, "", true, true);
}