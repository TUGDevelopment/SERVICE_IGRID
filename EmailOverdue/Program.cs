                    ...
                    using (CNService.IsolationLevel(context))
                    {
                            ...
                            try
                            {
                                ...
                                if (DateTime.Now.Date.AddDays(1).Date == finishDate.Date)
                                {
                                    if (item.CURRENT_VENDOR_ID > 0)
                                        EmailService.sendEmailMockup(item.MOCKUP_ID, item.MOCKUP_SUB_ID, "WF_OVERDUE_VN1", context);
                                    else if (item.CURRENT_CUSTOMER_ID > 0)
                                        EmailService.sendEmailMockup(item.MOCKUP_ID, item.MOCKUP_SUB_ID, "WF_OVERDUE_CUS1", context);
                                    else
                                        EmailService.sendEmailMockup(item.MOCKUP_ID, item.MOCKUP_SUB_ID, "WF_OVERDUE1", context);
                                }
                                else if (DateTime.Now.Date == finishDate.Date)
                                {
                                    if (item.CURRENT_VENDOR_ID > 0)
                                        EmailService.sendEmailMockup(item.MOCKUP_ID, item.MOCKUP_SUB_ID, "WF_OVERDUE_VN2", context);
                                    else if (item.CURRENT_CUSTOMER_ID > 0)
                                        EmailService.sendEmailMockup(item.MOCKUP_ID, item.MOCKUP_SUB_ID, "WF_OVERDUE_CUS2", context);
                                    else
                                        EmailService.sendEmailMockup(item.MOCKUP_ID, item.MOCKUP_SUB_ID, "WF_OVERDUE2", context);
                                }
                                else if (DateTime.Now.Date > finishDate.Date)
                                {
                                    if (item.CURRENT_VENDOR_ID > 0)
                                        EmailService.sendEmailMockup(item.MOCKUP_ID, item.MOCKUP_SUB_ID, "WF_OVERDUE_VN3", context);
                                    else if (item.CURRENT_CUSTOMER_ID > 0)
                                        EmailService.sendEmailMockup(item.MOCKUP_ID, item.MOCKUP_SUB_ID, "WF_OVERDUE_CUS3", context);
                                    else
                                        EmailService.sendEmailMockup(item.MOCKUP_ID, item.MOCKUP_SUB_ID, "WF_OVERDUE3", context);
                                }
                            }
                        ...
                        foreach (var item in allProcessArtwork)
                        {
                            try
                            {
                                ...
                                {
                                    if (item.CURRENT_VENDOR_ID > 0)
                                        EmailService.sendEmailArtwork(item.ARTWORK_REQUEST_ID, item.ARTWORK_SUB_ID, "WF_OVERDUE_VN1", context);
                                    else if (item.CURRENT_CUSTOMER_ID > 0)
                                        EmailService.sendEmailArtwork(item.ARTWORK_REQUEST_ID, item.ARTWORK_SUB_ID, "WF_OVERDUE_CUS1", context);
                                    else
                                        EmailService.sendEmailArtwork(item.ARTWORK_REQUEST_ID, item.ARTWORK_SUB_ID, "WF_OVERDUE1", context);
                                }
                                else if (DateTime.Now.Date == finishDate.Date)
                                {
                                    if (item.CURRENT_VENDOR_ID > 0)
                                        EmailService.sendEmailArtwork(item.ARTWORK_REQUEST_ID, item.ARTWORK_SUB_ID, "WF_OVERDUE_VN2", context);
                                    else if (item.CURRENT_CUSTOMER_ID > 0)
                                        EmailService.sendEmailArtwork(item.ARTWORK_REQUEST_ID, item.ARTWORK_SUB_ID, "WF_OVERDUE_CUS2", context);
                                    else
                                        EmailService.sendEmailArtwork(item.ARTWORK_REQUEST_ID, item.ARTWORK_SUB_ID, "WF_OVERDUE2", context);
                                }
                                else if (DateTime.Now.Date > finishDate.Date)
                                {
                                    if (item.CURRENT_VENDOR_ID > 0)
                                        EmailService.sendEmailArtwork(item.ARTWORK_REQUEST_ID, item.ARTWORK_SUB_ID, "WF_OVERDUE_VN3", context);
                                    else if (item.CURRENT_CUSTOMER_ID > 0)
                                        EmailService.sendEmailArtwork(item.ARTWORK_REQUEST_ID, item.ARTWORK_SUB_ID, "WF_OVERDUE_CUS3", context);
                                    else
                                        EmailService.sendEmailArtwork(item.ARTWORK_REQUEST_ID, item.ARTWORK_SUB_ID, "WF_OVERDUE3", context);
                                }
                            }
                            catch (Exception ex) { CNService.GetErrorMessage(ex); }
                        }
