...
{
    public class ItemPrintSystemHelper
    {
        public static SAP_M_CHARACTERISTIC_ITEM_2_RESULT GetPrintSystem(SAP_M_CHARACTERISTIC_ITEM_2_REQUEST param)
        {
                ...
                using (var context = new ARTWORKEntities())
                {
                    using (CNService.IsolationLevel(context))
                    {
                        if (param == null || param.data == null)
                        {
                            ...
                            characteristic.NAME = "PRINT_SYSTEM";
