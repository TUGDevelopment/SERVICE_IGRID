...
{
    public static class SD_129_Helper
    {
        public static void SaveLog(SAP_M_PO_COMPLETE_SO_MODEL param, string GUID)
        {
            try
            {
                using (var context = new ARTWORKEntities())
                {
                    ...
                    Item.ACTION = "Interface Inbound-SD129";
                    Item.TABLE_NAME = "SAP_M_PO_COMPLETE_SO";
        ...
        private static SERVICE_RESULT_MODEL aSavePOCompleteSO(SAP_M_PO_COMPLETE_SO_MODEL param)
        {
            ...
            CNService.SaveLogReturnInterface(Results, "SAP_M_PO_COMPLETE_SO", guid,"SD129");
        ...
        public static SERVICE_RESULT_MODEL aSavePOCompleteSOtmp(SAP_M_PO_COMPLETE_SO_MODEL param)
        {
            ...
            //---------------------------------
            DateTime dateStart, dateLast;
            int iCntHD, iCntITEM, iCntCOM;
            iCntHD = 1;
            //---------------------------------
            ...
            if (param != null)
            {
                try
                {
                    ...
                    {
                        context.Database.CommandTimeout = 600;
                        //var listSO_HEADER = param.SO_HEADERS.Select(m => m.SALES_ORDER_NO).Distinct().ToList();
                        //var listso = (from p in context.SAP_M_PO_COMPLETE_SO_HEADER
                        //              where listSO_HEADER.Contains(p.SALES_ORDER_NO)
                        //              select new SAP_M_PO_COMPLETE_SO_HEADER_2() { CREATE_BY = p.CREATE_BY, CREATE_DATE = p.CREATE_DATE, PO_COMPLETE_SO_HEADER_ID = p.PO_COMPLETE_SO_HEADER_ID, IS_MIGRATION = p.IS_MIGRATION, IS_ASSIGN = p.IS_ASSIGN }).ToList();
                        foreach (SO_HEADER itemSOHeader in param.SO_HEADERS)
                        {

                            //---------------------------------
                            dateStart = DateTime.Now;
                            Console.WriteLine(iCntHD + "." + itemSOHeader.SALES_ORDER_NO + "{Compare and Save Step}." );
                            iCntITEM = 0;
                            iCntCOM = 0;
                            //---------------------------------
                            ...
                            //var chkHeader = listso.Where(p => p.SALES_ORDER_NO == itemSOHeader.SALES_ORDER_NO).FirstOrDefault();
                            if (chkHeader != null)
                            {
                            ...
                            //
                            // set order
                            //result = context.Database.SqlQuery<int>("spsetassignsotmp @ID,@step",
                            //new SqlParameter("@ID", soHeader.PO_COMPLETE_SO_HEADER_ID.ToString()),
                            //new SqlParameter("@step", result.ToString())).FirstOrDefault();
                            //
                            ...
                            foreach (SO_ITEM itemSOItem in itemSOHeader.SO_ITEMS)
                            {
                                ...
                                soItemID = soItem.PO_COMPLETE_SO_ITEM_ID;
                                var tempCOMPONENT = SAP_M_PO_COMPLETE_SO_ITEM_COMPONENT_SERVICE.GetByItem(new SAP_M_PO_COMPLETE_SO_ITEM_COMPONENT() { 
                                    PO_COMPLETE_SO_ITEM_ID = soItem.PO_COMPLETE_SO_ITEM_ID }, context);
                                ...
                                //---------------------------------
                                iCntCOM = itemSOItem.COMPONENTS.Count;
                                //---------------------------------
                                foreach (COMPONENT itemSOComponent in itemSOItem.COMPONENTS)
                                {
                                    ...
                                    soItemComponent.UNIT = itemSOComponent.UNIT;
                                    soItemComponent.STOCK = itemSOComponent.STOCK;
                                    soItemComponent.BOM_ITEM_CUSTOM_1 = string.Format("{0}", itemSOComponent.BOM_ITEM_CUSTOM_1);
                                    soItemComponent.BOM_ITEM_CUSTOM_2 = itemSOComponent.BOM_ITEM_CUSTOM_2;
                                    soItemComponent.BOM_ITEM_CUSTOM_3 = itemSOComponent.BOM_ITEM_CUSTOM_3;
                                    soItemComponent.CREATE_BY = userID;
                                    soItemComponent.UPDATE_BY = userID;
                                    ...
                                    var chkComponent = (from p in context.SAP_M_PO_COMPLETE_SO_ITEM_COMPONENT
                                                        ...
                                                        select new SAP_M_PO_COMPLETE_SO_ITEM_COMPONENT_2() { CREATE_BY = p.CREATE_BY, 
                                                            CREATE_DATE = p.CREATE_DATE, 
                                                            PO_COMPLETE_SO_ITEM_COMPONENT_ID = p.PO_COMPLETE_SO_ITEM_COMPONENT_ID,
                                                            IS_ASSIGN=p.IS_ASSIGN }).FirstOrDefault();
                            ...
                            if (soHeader != null)
                            {
                                ...
                                //var h = listso.Where(s=> s.SALES_ORDER_NO == soHeader.SALES_ORDER_NO).FirstOrDefault();
                                if (h != null)
                                {
                    ...
                    try
                    {
                        ...
                        foreach (SO_HEADER itemSOHeader in param.SO_HEADERS)
                        {
                            using (var context = new ARTWORKEntities())
                            {
                                ...
                                //---------------------------------
                                dateStart = DateTime.Now;
                                Console.WriteLine(iCntHD + "." + itemSOHeader.SALES_ORDER_NO + "{FOC Step}.");
                             
                                //---------------------------------;
                                s = DateTime.Now;
                                DeleteFOC(itemSOHeader.SALES_ORDER_NO, context);
                                l = DateTime.Now;
                                CNService.consoleWriteLineProcessTime(s, l, "-----DeleteFOC");
                                ...
                                foreach (SO_ITEM itemSOItem in itemSOHeader.SO_ITEMS)
                                {
                                    foreach (COMPONENT itemSOComponent in itemSOItem.COMPONENTS)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(itemSOComponent.COMPONENT_MATERIAL) && string.IsNullOrEmpty(itemSOComponent.BOM_ITEM_CUSTOM_1))
                                        {
                                            if (listMat5.Where(m => m == itemSOComponent.COMPONENT_MATERIAL).ToList().Count() == 0)
                                            {
                                                ...
                                                l = DateTime.Now;
                                                CNService.consoleWriteLineProcessTime(s, l, "-----RepairMaterialBOM_1");
                                        ...
                                        else if (!string.IsNullOrEmpty(itemSOComponent.BOM_ITEM_CUSTOM_1))
                                        {
                                            ...
                                            l = DateTime.Now;
                                            CNService.consoleWriteLineProcessTime(s, l, "-----RepairMaterialBOM2_1");
                                        }
                                        //}
                                        //else
                                        //{
                                    ...
                                    l = DateTime.Now;
                                    CNService.consoleWriteLineProcessTime(s, l, "-----AddFOCIntoSOAssign");
                                }


                                //---------------------------------
                                dateLast = DateTime.Now;                          
                                CNService.consoleWriteLineProcessTime(dateStart, dateLast, iCntHD + ".Finished FOC");
                                iCntHD += 1;
                                //---------------------------------
                                CNService.DeleteSO(itemSOHeader.PO_COMPLETE_SO_HEADER_ID, context);

                                //---------------------------------
                                dateLast = DateTime.Now;
                                //Console.WriteLine("-----item:" + iCntITEM);
                                //Console.WriteLine("-----component:" + iCntCOM);
                                CNService.consoleWriteLineProcessTime(dateStart, dateLast, iCntHD + ".Finished Comparing and Deleting");
                                iCntHD += 1;
                                //---------------------------------
                            }
